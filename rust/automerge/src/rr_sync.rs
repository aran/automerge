//! # Request-Response (RR) Sync Protocol
//!
//! The sync protocol is intended to support sync over a request-response
//! protocol like HTTP.
//!
...
mod bloom;
mod rr_message_builder;
mod rr_state;
use rr_message_builder::MessageBuilder;

pub use bloom::{BloomFilter, DecodeError as DecodeBloomError};
pub use rr_state::DecodeError as DecodeStateError;
pub use rr_state::{Have, State};
...
impl SyncDoc for Automerge {
    fn generate_sync_message(&self, sync_state: &mut State) -> Option<Message> {
        ...
        let our_have = if our_need.iter().all(|hash| their_heads_set.contains(hash)) {
            vec![self.rr_make_bloom_filter(sync_state.shared_heads.clone())]
        } else {
            Vec::new()
        };
        ...
        let (message_builder, sent_hashes) = if let (Some(their_have), Some(their_need)) = (
            ...
            if send_doc {
                ...
                let all_changes = self
                    .rr_get_changes_to_send(their_have, their_need)
    ...
    fn receive_sync_message(
        ...
        self.rr_receive_sync_message_inner(sync_state, message, &mut patch_log)
    }
    ...
    fn receive_sync_message_log_patches(
        ...
        self.rr_receive_sync_message_inner(sync_state, message, patch_log)
    }
}

impl Automerge {
    fn rr_make_bloom_filter(&self, last_sync: Vec<ChangeHash>) -> Have {
    ...
    pub(crate) fn rr_receive_sync_message_inner(
        &mut self,
        sync_state: &mut State,
        message: Message,
        patch_log: &mut PatchLog,
...
mod tests {
    ...
    fn first_response_is_some_even_if_no_changes() {
        ...
        doc2.rr_sync().receive_sync_message(&mut s2, m1).unwrap();
    ...
    fn should_not_reply_if_we_have_no_data_after_first_round() {
        ...
        let m1 = doc1.rr_sync().generate_sync_message(&mut s1);
        assert!(m1.is_none());

        let m2 = doc2.rr_sync().generate_sync_message(&mut s2);
        assert!(m2.is_none());
    }
    ...
    fn should_allow_simultaneous_messages_during_synchronisation() {
        ...
        assert!(doc1.rr_sync().generate_sync_message(&mut s1).is_none());
        assert!(doc2.rr_sync().generate_sync_message(&mut s2).is_none());
    ...
    fn in_flight_logic_should_not_sabotage_concurrent_changes() {
        ...
        for _ in 0..300 {
            ...
            let msg = doc2.rr_sync().generate_sync_message(&mut s2).unwrap();
            // Receive that sync message on doc1
            doc1.rr_sync().receive_sync_message(&mut s1, msg).unwrap();
    ...
    fn sync(
        ...
        loop {
            let a_to_b = a.rr_sync().generate_sync_message(a_sync_state);
            let b_to_a = b.rr_sync().generate_sync_message(b_sync_state);
            ...
            if let Some(msg) = a_to_b {
                b.rr_sync().receive_sync_message(b_sync_state, msg).unwrap()
            }
            if let Some(msg) = b_to_a {
                a.rr_sync().receive_sync_message(a_sync_state, msg).unwrap()
            }
            iterations += 1;
        }
    }
